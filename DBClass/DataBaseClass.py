...
class ApplicantTable(DataBase):
    ...
    def update_category_questions(self, code: int, email: str) -> None:
...
class TokenTable(DataBase):
    ...
    def remove_expired_tokens(self, list_save_temp: list) -> None:  # Возможно ли DELETE FROM token WHERE save_temp IN {tuple(list_save_temp)}
...
class AnswerTable(DataBase):
    ...
    def insert_answer_applicants(self, applicant_id: int, value_answer_dict: list, question_id: list) -> None:
        """Добавляет ответы соискателя"""
        for i in range(len(value_answer_dict)):  # Как добавить без цикла?
            self.cur.execute(f"INSERT INTO answer VALUES ({applicant_id}, '{value_answer_dict[i]}', {question_id[i]})")
            self.conn.commit()

    def check_answer_applicants(self, applicant_id: int) -> list:
        """Выбирает id соискателя и id вопросов по id соискателя"""
...
class QuestionTable(DataBase):
    ...
    def get_text_questions(self, random_id: int) -> list:
        ...
        self.cur.execute(f"SELECT quest_text, pseudonym FROM question WHERE question_list_id = {random_id} ORDER BY id")
        list_tuples = self.cur.fetchall()
        return list_tuples

    def select_list_question_id(self, code: int) -> list:
        """Выбирает id вопросов по code"""
    ...
    def get_question_text_by_id_question(self, id_questions: list) -> list:
        ...
        list_tuple = []
        for i in range(len(id_questions)):  # Если делать через IN (tuple) - считает от первого id, Важен порядок вывода в list_tuple
            self.cur.execute(f"SELECT quest_text FROM question WHERE id = {id_questions[i]}")
            list_tuple.append(self.cur.fetchone()[0])
        return list_tuple

    def get_question_id_and_question_pseudonym(self, key_answer_pseudonym: list) -> list:
        """Выбирает id вопроса и псевдоним вопроса по псевдонимам, важен порядок"""
        list_tuple = []
        for i in range(len(key_answer_pseudonym)):  # Важен порядок вывода в list_tuple
            self.cur.execute(f"SELECT id, pseudonym FROM question WHERE pseudonym = '{key_answer_pseudonym[i]}'")
            list_tuple.append(self.cur.fetchone()[0])
        return list_tuple
